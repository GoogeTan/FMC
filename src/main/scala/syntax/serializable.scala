...
import data.NBT.{Compound, End}
import data.{NBT, Serializable}
    ...
    override def deserialize(container: NBT): Option[List[T]] =
      container match
        case tag :  NBT.Compound =>
          for
            length <- data.get[Int]("size", tag)
            values <- (0 to length).map(_.toString).map(data.get[T](_, tag)).foldRight(Option(List[T]())) {
